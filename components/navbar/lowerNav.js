...
import { Menu, Row, Col, Icon, Drawer } from "antd";
...
const links = [
  ...
  <Item key={link.as || link.href}>
    <Link href={link.href} prefetch={false} as={link.as || link.href}>
      <a className={link.className || ""}>{link.title}</a>
    </Link>
  </Item>
));
...
const LowerNav = props => {
  ...
  const selectedPath =
    router.pathname !== "/_error" ? router.pathname : router.asPath;
  return (
      ...
      <div className="container">
        {!isMobile ? (
          <Row type="flex">
            ...
            <Col className="text-right" sm={24} md={10}>
              <Menu mode="horizontal">
                <SubMenu
                  title={
                    <span className="submenu-title-wrapper secondary-color bold">
                ...
                <Item>
                  <Link href="/cart" as="shopping-cart" prefetch={false}>
                    <a className="flex">
          ...
          <div>
            ...
            <Drawer placement="left" visible={toggled} onClose={toggleMenu}>
              <Menu mode="vertical">
                {links}
                <Item key="setting:1">
                  <Link href="/login">
